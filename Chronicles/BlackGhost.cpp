...
BlackGhost::BlackGhost()
{
	...
	sprite_.setTextureRect(sf::IntRect(
		texture_.getSize().x * 9 / 12, texture_.getSize().y *5/ 8, texture_.getSize().x / 12, texture_.getSize().y / 8));
	...
	onFloor_ = false;
	attacked_ = false;
	movementSpeed_ = sf::Vector2f(6.f, 6.f);
...
void BlackGhost::update(sf::Time elapsed)
{
	if (!dead) {
		...
		if (speed_.x > 0) {
			sprite_.setTextureRect(sf::IntRect(
				texture_.getSize().x * 9 / 12, texture_.getSize().y * 6 / 8, texture_.getSize().x / 12, texture_.getSize().y / 8));
		}
		if (speed_.x == 0) {
			sprite_.setTextureRect(sf::IntRect(
				texture_.getSize().x * 9 / 12, texture_.getSize().y * 4 / 8, texture_.getSize().x / 12, texture_.getSize().y / 8));
		}
		sprite_.setPosition(box_.getPosition());
	}
}

void BlackGhost::render(sf::RenderWindow* window)
{
	if (this->dead) {
		died();
...
void BlackGhost::attack()
{
}

void BlackGhost::move(sf::Vector2f movement)
{
